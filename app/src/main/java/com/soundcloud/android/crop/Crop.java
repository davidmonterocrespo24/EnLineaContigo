...
public class Crop {
    ...
    public static void takeImage(Activity activity) {
        pickImage2(activity, REQUEST_PICK);
    }
     ...
     * @param requestCode requestCode for result
     */
    public static void pickImage2(Activity activity, int requestCode) {
        try {
            activity.startActivityForResult(getTakeImage(), requestCode);
        } catch (ActivityNotFoundException e) {
            showImagePickerError(activity);
        }
    }
